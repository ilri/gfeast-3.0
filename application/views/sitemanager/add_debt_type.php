        ...
        <div class="col-md-12 mt-10">
          <div class="card p-10">
            ...
            <form  action="javascript:void(0);" id="form">
              <input type="hidden" name="debt_type_id" class='debt_type_id' value="<?php if(!empty($debt_type)){echo $debt_type[0]->id;}?>">
              <div class="row">
                <div class="col-md-3">
                  <label>Name</label>
                  <input type="text" name="debt_type_name" class="name form-control" placeholder="Enter debt_type name here"  value="<?php if(!empty($debt_type)){ echo $debt_type[0]->name;}?>">
                ...
                <div class="col-md-3">
                  <!-- <label>Name</label>
                  <input type="text" name="name"  class="name form-control" placeholder="Enter name here" value="<?php if(!empty($debt_type)){ $debt_type[0]->name;}?>"><span class="latitude_err"></span> -->
                </div>
                <div class="col-md-3">
                  <!-- <label>longitude</label>
                  <input type="text" name="longitude"  class="longitude form-control" placeholder="Enter longitude here" value="<?php if(!empty($debt_type)){ $debt_type[0]->lng;}?>" >
...
<script>
  $(function(){
    $('#submit').click(function(){
      var debt_type_id=$('.debt_type_id').val(); 
      var debt_type_name=$('.name').val();

      // console.log(debt_type_id+"  "+debt_type_name);
      var error=0;
      if(debt_type_name==""){
        var error=error+1;
      ...
      if(debt_type_name!="" && error=='0'){
         console.log("debt_type_name is not null"+debt_type_name);
        $.ajax({
          url:"<?php echo base_url();?><?php echo $this->uri->segment(1);?>/adddebt_type",
          type:'post',
          dataType:'json',
          data:{
