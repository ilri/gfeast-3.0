  ...
  <div class="content-wrapper">
    <div class="content-body">
        ...
        <div class="col-md-12">
          ...
          <?php if(!empty($subcounty[0]->county_id)){?>
    ...
    <div class="card shadow-none" style="margin-top:20px;margin-bottom:90px;">
        <div class="card-body">
         <a class="btn btn-md btn-round btn-success pull-right addsubcounty" href="javascript:void(0);">
            <i class="fa fa-plus " aria-hidden="true"></i> SubCounty
          </a>
          <h5 class="card-title">Sub County Details</h5>
          <div class="table-responsive">
            <table class="table mb-0" id="test">
              <thead>
                <tr>
                  <th scope="col">Sl.NO</th>
                  <th scope="col">Subcounty Id</th>
              ...
              <tbody>
                <?php  foreach ($subcounties as $key =>  $subcounty) {?>
                  <tr>
                    <td class="text-truncate"><?php echo $key+1; ?></td>
                    <td class="text-truncate" ><?php echo $subcounty->sub_county_id;?></td>
                    <td class="text-truncate"><?php echo $subcounty->sub_county_name;?></td>
                   <!--  <td class="text-truncate"><?php echo $country->country_code; ?></td> -->
                    <td style="display: none;" class="status_<?php echo $subcounty->sub_county_id;?>">
                      <input type="hidden" name="status" class="status" value="<?php echo $subcounty->status;?>">
                      <?php if($subcounty->status=='1'){?>
                    ...
                    <td class="text-truncate" class="status2_<?php echo $subcounty->sub_county_id;?>">
                      <?php if($subcounty->status=='1'){?>
                      <a href="javascript:void(0);" class="delete" id="<?php echo $subcounty->sub_county_id;?>">
                        <i style="font-size:25px;color:green" class="togg<?php echo $subcounty->sub_county_id;?> <?php echo "fa fa-toggle-on"; ?>"></i>
                      </a><?php }else{ ?>
                      <a href="javascript:void(0);" class="delete" id="<?php echo $subcounty->sub_county_id;?>">
                        <i style="font-size:25px;color: red" class="togg<?php echo $subcounty->sub_county_id;?> <?php echo "fa fa-toggle-off"; ?>"></i>
                      </a><?php } ?>
                    </td>
                    <td>
                      <a href="<?php echo base_url();?><?php echo $this->uri->segment(1);?>/subcounty_edit/<?php echo $subcounty->sub_county_id;?>" class="edit" id="<?php echo $subcounty->sub_county_id;?>">
                        <i class="fa fa-edit" style="font-size:25px;"></i>
                      </a>
                      <a href="<?php echo base_url();?><?php echo $this->uri->segment(1);?>/get_ward/<?php echo $subcounty->sub_county_id;?>">
...
<script>
  $(function(){
    ...
    $('.delete').click(function(){
      var subcounty_id=$(this).attr('id');
      var subcounty_status=$(this).parent().prev().find('.status').val();
      if(subcounty_id!="" && subcounty_status!=""){
        $.ajax({
          url : '<?php echo base_url();?><?php echo $this->uri->segment(1);?>/subcounty_delete',
          type : 'POST',
          dataType:'json',
          data : {
            subcounty_id:subcounty_id,
            subcounty_status:subcounty_status,
          },
          success : function(data){	
            if(data.cstatus==1){
              $('.status_'+subcounty_id).html("");
              $('#test tbody').find('.status_'+subcounty_id).html('<input type="hidden" name="status" class="status" value="'+data.cstatus+'"><i style="font-size:25px;color:green;" class="fa fa-check"></i>');
               $(".togg"+subcounty_id).removeClass('fa fa-toggle-off');
              $('.togg'+subcounty_id).addClass('fa fa-toggle-on "togg'+subcounty_id+'"').css("color","green");
            }
            else{
              $('.status_'+subcounty_id).html("");
              $('#test tbody').find('.status_'+subcounty_id).html('<input type="hidden" name="status" class="status" value="'+data.cstatus+'"><i style="font-size:25px;color:red;" class="fa fa-close"></i>');
              $(".togg"+subcounty_id).removeClass('fa fa-toggle-on');
              $('.togg'+subcounty_id).addClass('fa fa-toggle-off "togg'+subcounty_id+'"').css("color","red");
            }
          },
        });
      }
