...
<!-- User Centre Combo Modal -->
<div class="modal fade" id="manageCentre" role="dialog">
	<div class="modal-dialog modal-md">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title">Manage centre-user details</h4>
			</div>
			...
			<?php echo form_open('', array('id' => 'centreForm')) ?>
				<div class="modal-body">
					...
					<div class="form-group">
						<label for="users">Select Users</label> <span class="text-danger">*</span>
						<select name="users[]" id="users" multiple="multiple"></select>
						<span class="users error text-danger"></span>
					</div>
				</div>
				<div class="modal-footer">
					<button type="submit" class="btn btn-info">Update Centre-User Details</button>
											...
											<?php if(count($all_centre) > 0){
												foreach ($all_centre as $key => $centre) { ?>
													<tr>
														...
														<td><a href="javascript:void(0);" class="btn btn-primary btn-sm manage" data-id="<?php echo $centre['centre_id']; ?>">Manage Users</a></td>
													</tr>
												<?php }
											}else{ ?>
												<tr>
...
<script type="text/javascript">
	...
	$('body').on('click', '.manage', function(event) {
		...
		$('#manageCentre').find('#users').val(null).empty();
		...
		$.ajax({
			...
			error: function() {
				$.toast({
					...
					afterHidden: function () {
						...
						$('#manageCentre').find('button[type="submit"]').html('Update Centre-User Details');
					}
				});
			},
			success: function(data) {
				...
				$('#manageCentre').find('#users').html(HTML).val(null);
				if(data.details.users.length > 0) {
					$('#manageCentre').find('#users').val(data.details.users);
				}
			}
		});
	});
	...
	$('#centreForm').on('submit', function(event) {
		...
		$.ajax({
			url: '<?php echo base_url(); ?>centre/manage_user/',
			type: 'POST',
			data: formData,
			processData: false,
			contentType: false,
			...
			error: function() {
				...
				$('button[type="submit"]').html('Update Centre-User Details');
				$.toast({
					heading: 'Network Error!',
					text: 'Could not establish connection to server. Please refresh the page and try again.',
					icon: 'error'
			...
			success: function(data) {
				...
				if(data.updatestatus == 1) {
					...
					$.toast({
						...
						icon: 'success',
						afterHidden: function () {
							window.location.href = '<?php echo base_url(); ?>centre/user';
						}
					});
